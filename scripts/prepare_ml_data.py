...
def catalog_add(catalog, sku, name="", unit="", supplier_sku=""):
    ...
    for key, raw in [("product_name", name), ("unit", unit), ("supplier_sku", supplier_sku)]:
        text = clean_text(raw)
        if key == "unit" and text and record[key] and record[key] != text:
            raise ValueError(f"{sku}: несовместимые единицы {record[key]!r} и {text!r}")
...
def read_transactions(path, catalog, issues):
    ...
    with table(path, "Дата") as (headers, rows):
...
def read_transit(path, catalog, issues):
    ...
    with table(path, "Код 1с") as (headers, rows):
        ...
        for rn, row in rows:
            ...
            # В реальном файле есть заметки (1, 1, пусто) и (0, "Расширение...").
            # Исключаем только строки без названия и любых количеств поставки.
            if (raw_code is not None and not isinstance(raw_code, str)
                    and not clean_text(value(row, ni))
                    and all(not clean_text(value(row, col)) for col in arrivals)):
...
def read_seasonality(path, issues):
    ...
    with table(path, "СЕЗОННОСТЬ") as (headers, rows):
