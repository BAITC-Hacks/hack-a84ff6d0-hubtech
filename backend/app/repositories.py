...
def public_catalog_summary(report: dict) -> dict:
    """Only expose bounded coverage facts, never crawler diagnostics or paths."""
    result = {}
    if isinstance(report.get("status"), str) and report["status"] in {"disabled", "missing", "invalid", "empty_catalog", "partial", "loaded"}:
        result["status"] = report["status"]
    if isinstance(report.get("source"), str) and report["source"] in {"https://ekt.kz", "https://ekt.kz/", "https://www.ekt.kz", "https://www.ekt.kz/"}:
        result["source"] = report["source"]
    fetched = report.get("fetched_at")
    if fetched is None:
        result["fetched_at"] = None
    elif isinstance(fetched, str) and len(fetched) <= 80 and ("T" in fetched or " " in fetched):
        try:
            datetime.fromisoformat(fetched.replace("Z", "+00:00"))
            result["fetched_at"] = fetched
        except ValueError:
            pass
    counters = ("total_catalog", "web_products", "valid_products", "matched", "unmatched", "conflicts",
                "invalid_products", "duplicate_skus", "duplicate_products", "unknown_brand_matches",
                "crawl_quarantined", "crawl_failures")
    for key in counters:
        value = report.get(key)
        if type(value) is int and 0 <= value <= 2 ** 53 - 1:
            result[key] = value
    coverage = report.get("crawl_coverage")
    if coverage is None or coverage in ("partial", "complete"):
        result["crawl_coverage"] = coverage
    reasons = report.get("conflict_reasons")
    if isinstance(reasons, dict):
        result["conflict_reasons"] = {
            key: value for key, value in reasons.items()
            if key in {"supplier_sku", "brand", "duplicate_web_sku", "duplicate_base_sku"}
...
def public_metadata(metadata: dict) -> dict:
    ...
    if isinstance(metadata.get("catalog_enrichment"), dict):
        result["catalog_enrichment"] = public_catalog_summary(metadata["catalog_enrichment"])
