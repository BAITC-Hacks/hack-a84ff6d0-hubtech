...
def _template(name: str, r: Rationale, qty: float, urgency: str) -> str:
    parts = [
        f"«{name}»: рекомендуем заказать {qty:g} ед.",
        f"Прогноз спроса на {r.horizon_days} дн. — {r.forecast_demand:g} ед.",
    ]
    ...
    if r.lost_demand_uplift > 0:
        parts.append(f"учтён упущенный спрос за периоды отсутствия (+{r.lost_demand_uplift:g} ед.)")
    if r.excluded_bulk_orders > 0:
        parts.append(
            f"исключены разовые оптовые заказы ({r.excluded_bulk_orders} шт., "
            f"{r.excluded_bulk_units:g} ед.), чтобы не завышать регулярную потребность"
        )
    parts.append(
        f"страховой запас {r.safety_stock:g} ед.; вычтены остаток {r.on_hand:g} "
        f"и товары в пути {r.in_transit:g}."
    )
    return " ".join(parts)


def build_explanation(name: str, r: Rationale, qty: float, urgency: str, use_llm: bool) -> str:
    """Возвращает человекочитаемое обоснование строки заказа."""
    base = _template(name, r, qty, urgency)
    ...
    try:
        ...
        prompt = (
            ...
            f"Товар: {name}\nРекомендовано к заказу: {qty:g} ед.\nСрочность: {urgency}\n"
