...
def ready():
    try:
        ...
        return JSONResponse({"status": "ok" if worker else "unavailable", "worker": worker}, status_code=200 if worker else 503)
    except sqlite3.Error:
        return JSONResponse({"status": "unavailable", "worker": False}, status_code=503)
...
def recommend_export(req: ExportRequest, session=Depends(current_session)) -> Response:
    with connection() as db:
        ...
        order = repo.authorized_order(db, row["id"], session["user"])
    try:
        snapshot = RecommendationResponse.model_validate_json(order["calculation"])
        data = to_excel_bytes(snapshot, req)
    except ExportValidationError as error:
        raise HTTPException(422, str(error)) from None
    with connection(write=True) as db:
        repo.event(db, row["id"], session["user"], "legacy_exported", {"approved_only": req.approved_only})
    return _xlsx(data, req.approved_only)
