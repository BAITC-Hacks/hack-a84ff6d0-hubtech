...
class ServiceTests(unittest.TestCase):
    ...
    def test_readiness_requires_worker_and_accounting_source_files(self):
        register('readiness-worker')
        with patch('app.api.routes.source_files_available', return_value=False):
            result = self.client.get('/api/ready')
        self.assertEqual(result.status_code, 503)
        self.assertEqual(result.json(), {'status': 'unavailable', 'worker': True, 'data': False})
        with patch('app.api.routes.source_files_available', return_value=True):
    ...
    def test_calculation_rejects_unknown_fields_and_coerced_parameters(self):
        for payload in ({'explain': 'false'}, {'review_period_days': True}, {'service_level': '0.95'}, {'unexpected': 1}):
            with self.subTest(payload=payload):
                self.assertEqual(self.client.post('/api/recommend', json=payload).status_code, 422)

    def test_unknown_unit_cannot_be_confirmed_in_saved_order(self):
        calculation = example_response()
        calculation.groups[0].lines[0].unit = 'ед. (не указана)'
        with connection(write=True) as db:
            order_id = create_order(db, calculation, self.manager)
        detail = self.client.get('/api/orders/' + order_id).json()
        detail['decisions'][0]['approved'] = True
        response = self.client.put('/api/orders/' + order_id,
                                   json={'revision': 1, 'lines': detail['decisions']})
        self.assertEqual(response.status_code, 422)
        self.assertIn('единицу измерения', response.json()['detail'])
        unchanged = self.client.get('/api/orders/' + order_id).json()
        self.assertEqual(unchanged['revision'], 1)
        self.assertFalse(unchanged['decisions'][0]['approved'])
    ...
    def test_worker_recovery_fails_running_keeps_queue(self):
        ...
        # created_at has millisecond precision and ties are ordered by UUID.
        # Explicitly start the orphan before enqueuing the survivor: this tests
        # recovery of running vs queued work, not incidental enqueue timing.
        self.assertEqual(jobs.claim("worker-one"), first["job_id"])
