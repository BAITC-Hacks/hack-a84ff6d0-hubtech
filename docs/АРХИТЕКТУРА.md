# Архитектура сервиса автозаказов поставщикам (кейс ЭКТ)

> Как система устроена и как должна выглядеть: слои, потоки данных, модель Dataset, sequence расчёта, деплой.

---

## 1. Принцип решения

Менеджер закупа считает потребность вручную в Excel — редко и неточно, из-за чего возникают избыток по одним позициям и дефицит по другим. Сервис автоматизирует расчёт: читает реальные выгрузки 1С, строит прогноз спроса с сезонностью и трендом, в дневном режиме компенсирует упущенный спрос при наличии явных интервалов, исключает разовый опт и формирует **рекомендованный заказ по каждому поставщику** с обоснованием. Итоговое решение остаётся за человеком — заказ утверждается вручную.

Ключевой архитектурный выбор: **месячные итоги 1С приняты основным источником количества до сверки с партнёром**, транзакции служат для выявления исключительных (оптовых) заказов. Все окна прогноза и ETA — полуоткрытые `[as_of, as_of + H)`, единые по всему расчёту.

---

## 2. Слоистая архитектура (целевая)

```mermaid
flowchart TD
    subgraph L1["① Источники данных"]
        E["Excel-выгрузки 1С<br/>IEK · Systeme Electric"]
        C["CSV-выгрузки<br/>(предконвертация)"]
        S["Синтетика<br/>(демо/тесты)"]
        W["ekt.kz → отдельная синхронизация<br/>data/ekt/catalog.json"]
    end
    subgraph L2["② Слой данных (adapter)"]
        IMP["ExcelDataSource / CsvDataSource / SyntheticDataSource"]
        DS[("Dataset<br/>catalog · sales · monthly_sales/stock<br/>stock · in_transit · seasonality · warnings")]
    end
    subgraph L3["③ Движок расчёта (core)"]
        O["outliers — исключение опта"]
        K["stockout — компенсация спроса"]
        F["forecasting — сезонность+тренд"]
        R["replenishment — потребность"]
        RC["recommend — оркестратор + группировка"]
        EX["explain — обоснование (OpenAI API)"]
    end
    subgraph L4["④ API (FastAPI)"]
        API["/api/meta · /api/recommend · /api/recommend/export"]
        SNP["snapshots — расчёт по calculation_id"]
    end
    subgraph L5["⑤ Клиент (React/Vite)"]
        UI["Дашборд: фильтры · таблица по поставщикам<br/>раскладка · корректировка · утверждение · экспорт"]
    end
    E & C & S --> IMP --> DS
    W -->|точный SKU, описательные поля| DS
    E --> ETL["scripts/ — очистка и ML-подготовка IEK"]
    ETL --> MLFILES["data/clean/iek · data/ml/iek<br/>локальные таблицы и аудит"]
    MLFILES -.-> FUTURE["Будущее обучение и оценка<br/>не подключено к расчёту"]
    DS --> O --> F --> R --> RC --> EX
    O -.->|дневная история с интервалами| K --> F
    RC --> API --> SNP
    API --> UI
    EX --> API
```

**Границы слоёв:** движок работает с контрактом `Dataset`. Новый источник должен согласовать поля, даты, единицы и полноту данных с этим контрактом. Внешний каталог не меняет учётные числа; расчёт и экспорт не запрашивают сайт. 1С не подключена напрямую. Контракты: [ИНТЕГРАЦИЯ_1С.md](ИНТЕГРАЦИЯ_1С.md), [КАТАЛОГ_EKT.md](КАТАЛОГ_EKT.md).

---

## 3. Технологический стек

| Слой | Технологии |
|---|---|
| Данные / ETL | Python, **pandas**, **openpyxl** (read-only, `data_only`) |
| Прогноз | **numpy** (линейный тренд), **scipy** (квантиль); statsmodels не используется текущей моделью |
| API | **FastAPI**, **uvicorn**, **pydantic v2** |
| LLM | **OpenAI API** (обоснования; офлайн-фолбэк без ключа) |
| Экспорт | **openpyxl** (Excel для сверки; обратная загрузка в 1С не реализована) |
| Фронтенд | **React 19**, **Vite**, нативный fetch |
| Тесты | **unittest** (импорт, расчёт, экспорт; можно запускать через pytest), **node --test** для orderUtils |

---

## 4. Поток данных end-to-end

```mermaid
flowchart LR
    A["xlsx 1С"] -->|openpyxl read-only| B["Импорт: нормализация кодов,<br/>парсинг месяцев/ETA, сверка"]
    B --> C["Dataset + warnings + metadata"]
    C -->|per SKU×склад| D["Очистка: исключить опт,<br/>stockout только в дневном режиме"]
    D --> E["Прогноз: уровень × сезонность × тренд"]
    E --> F["Потребность = прогноз(H) + SS − остаток − в пути"]
    F --> G["Группировка по поставщику + обоснование"]
    G --> H["Snapshot + JSON-ответ"]
    H --> I["React: таблица, корректировка, утверждение"]
    I -->|approved_only| J["Excel для сверки и передачи"]
```

---

## 5. Модель данных (Dataset)

Единый контейнер `app/data/adapter.py::Dataset` — набор DataFrame'ов:

```mermaid
erDiagram
    CATALOG ||--o{ SALES : "sku"
    CATALOG ||--o{ MONTHLY_SALES : "sku"
    CATALOG ||--o{ MONTHLY_STOCK : "sku"
    CATALOG ||--o{ STOCK : "sku"
    CATALOG ||--o{ IN_TRANSIT : "sku"
    CATALOG }o--|| SKU_SUPPLIERS : "sku→supplier"
    SUPPLIERS ||--o{ SKU_SUPPLIERS : "supplier_id"
    SUPPLIERS ||--o{ SEASONALITY : "supplier_id"

    CATALOG {
        string sku PK "код 1С"
        string name
        string category "внутренняя 1С"
        string product_category "каталог ekt.kz"
        string product_url
        string unit
        string supplier_id
    }
    SALES {
        date date
        string sku
        float qty
        string warehouse
        string order_id
    }
    MONTHLY_SALES {
        string sku
        date month
        float qty
        float raw_qty
    }
    MONTHLY_STOCK {
        string sku
        date month
        float on_hand
        float raw_qty
    }
    STOCK {
        string sku
        string warehouse
        float on_hand
        date as_of
        string source
    }
    IN_TRANSIT {
        string sku
        float qty
        date eta
        date source_as_of
    }
    SUPPLIERS {
        string supplier_id PK
        int lead_time_days
        int min_order_qty
    }
    SKU_SUPPLIERS {
        string sku FK
        float pack_size
        float min_order_qty
        bool constraint_known
    }
    SEASONALITY {
        string supplier_id
        int month
        float factor
    }
```

Дополнительно `Dataset` несёт `as_of` (дата расчёта), `source` (`excel`/`csv`/`synthetic`), `warnings` (допущения для UI) и `metadata` (счётчики качества данных).

---

## 6. Ролевая модель

Кейс однопользовательский на MVP: основной пользователь — **менеджер отдела закупа**. Разграничение доступа/аутентификация в MVP не реализованы (см. НФТ и Roadmap). Целевая роль руководителя (утверждение крупных заказов) описана в [КОНЦЕПЦИЯ_СИСТЕМЫ_РК.md](КОНЦЕПЦИЯ_СИСТЕМЫ_РК.md).

---

## 7. Сценарий «расчёт заказа» (последовательность)

```mermaid
sequenceDiagram
    actor M as Менеджер закупа
    participant UI as React
    participant API as FastAPI
    participant ENG as Движок
    participant LLM as OpenAI API
    M->>UI: выбрать склад/категорию, «Рассчитать»
    UI->>API: POST /api/recommend
    API->>ENG: generate_recommendations(ds, фильтры)
    ENG->>ENG: по SKU×склад: сверка, опт, дневная/месячная модель, потребность
    ENG->>LLM: обоснование (если explain=true и задан ключ)
    LLM-->>ENG: текст (или шаблон при сбое/без ключа)
    ENG-->>API: RecommendationResponse
    API->>API: save_snapshot(calculation_id)
    API-->>UI: заказы по поставщикам + calculation_id
    M->>UI: корректировка кол-ва, утверждение
    UI->>API: POST /api/recommend/export {calculation_id, approved_only, lines}
    API-->>UI: Excel (approved_order.xlsx)
```

---

## 8. Схема развёртывания

```mermaid
flowchart TD
    subgraph Server["VPS / контейнер"]
        UV["uvicorn :8017<br/>FastAPI (app.main:app)"]
        FILES["IEK/ · Systeme electric/<br/>(выгрузки 1С)"]
        STATIC["frontend/dist (статика)"]
    end
    Browser["Браузер менеджера"] -->|HTTPS| NGINX["nginx (reverse proxy)"]
    NGINX --> UV
    UV --> FILES
    NGINX --> STATIC
    UV -.->|опц.| OPENAI["OpenAI API"]
```

Детали — в [DEPLOY.md](DEPLOY.md).

---

## 9. Структура репозитория

```
backend/app/
  main.py                 FastAPI: CORS, роутер /api
  config.py               DATA_SOURCE, DATA_DIR, сроки поставки, уровень сервиса
  schemas.py              Pydantic: Sale, Supplier, OrderLine, Rationale, Recommend/ExportRequest
  api/routes.py           /health /meta /recommend /recommend/export
  core/
    outliers.py           MH#4 — исключение разовых оптовых заказов
    stockout.py           MH#3 — компенсация упущенного спроса
    forecasting.py        MH#2 — прогноз (сезонность + тренд)
    replenishment.py      MH#1 — точка заказа + страховой запас
    recommend.py          оркестратор + MH#5 (группировка по поставщикам)
    explain.py            обоснование строки (OpenAI API + шаблон)
    export.py             экспорт заказа в Excel
    snapshots.py          хранение расчёта по calculation_id для экспорта
  data/
    adapter.py            Dataset + фабрика источника
    excel.py              импорт реальных выгрузок 1С (IEK, Systeme Electric)
    synthetic.py          генератор данных для демо/тестов
  tests/                  test_excel_import, test_calculation, test_export, validate_must_have
scripts/
  clean_monthly_data.py   отдельная очистка месячных таблиц IEK
  prepare_ml_data.py      признаки, разбиения, сверка и отчёт качества
  test_*.py              проверки подготовки; не входят в backend discovery
frontend/src/
  App.jsx, App.css        дашборд
  api.js                  клиент REST
  orderUtils.js(.test.js) утилиты подсчёта/валидации заказа
```

---

## 10. Нефункциональные требования

| НФТ | Как обеспечено |
|---|---|
| **Приватность** | Клиентские данные обезличены; в выгрузках 1С клиентов нет — опт выявляется по транзакциям |
| **Объяснимость** | Раскладка расчёта + обоснование по каждой строке; предупреждения и происхождение данных |
| **Устойчивость к выбросам** | Robust-детекция (Тьюки/медиана), клиппинг отрицательных, сверка транзакций с месяцами |
| **Обмен с учётной системой** | Чтение конкретных Excel и согласованных CSV; Excel-экспорт. Импорт заказа в конкретную 1С ещё не реализован и не проверен |
| **Безопасность решений** | Заказ не отправляется автоматически — только рекомендация + ручное утверждение |
| **Воспроизводимость** | Кеш импорта по fingerprint файлов; детерминированная синтетика (seed) |
| **Производительность** | Группировка таблиц один раз; фактическое время зависит от объёма и окружения |

## 11. Что требует отдельного внедрения

Расписание получения учётных данных, прямое чтение HTTP/OData конкретной базы 1С и обратная загрузка заказа — последующие интеграции. Наличие файлового импорта и JSON-обогащения не означает их подключения. В прототипе достаточно локальных файлов; схема согласования производственного канала приведена в [ИНТЕГРАЦИЯ_1С.md](ИНТЕГРАЦИЯ_1С.md).

## 12. Контур исследования и региональные данные

Скрипты из проверенной ветки `alish` работают отдельно от `DataSource → Dataset → recommend`. Они читают те же исходные IEK-файлы, создают локальные производные данные, но не меняют источник API. Подготовленная таблица признаков не является обученной моделью. Подключение будущего прогнозного метода потребует собственного контракта, проверки по времени и сравнения с текущим алгоритмом: [АУДИТ_ВЕТКИ_ALISH.md](АУДИТ_ВЕТКИ_ALISH.md).

Справочник складов API объединяет непустые значения `sales`, `monthly_sales`, `stock` и `in_transit`. Город публичного сайта и склад 1С не считаются одним идентификатором: необходима явная связь и согласованные учётные данные. Сейчас реальные транзакции относятся к Алматы, а месячные матрицы и путь назначены ему с предупреждением. План регионального подключения — [ГОРОДА_И_СКЛАДЫ.md](ГОРОДА_И_СКЛАДЫ.md).
