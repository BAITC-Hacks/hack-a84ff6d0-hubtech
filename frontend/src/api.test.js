...
test('display fields cannot carry objects or malformed server order metadata', () => {
  ...
  for (const field of ['unit', 'category', 'supplier_sku', 'warehouse', 'product_category', 'product_url', 'catalog_fetched_at']) {
    const changed = { ...line, [field]: {} }
    assert.equal(isRecommendation({ ...calculation, groups: [{ ...calculation.groups[0], lines: [changed] }] }), false)
  }
  assert.equal(isRecommendation({ ...calculation, groups: [{ ...calculation.groups[0], lines: [{ ...line, product_attributes: { voltage: {} } }] }] }), false)
})
