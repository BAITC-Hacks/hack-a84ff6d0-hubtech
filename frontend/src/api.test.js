...
import { order as uiOrder } from './test/fixtures.js'
...
test('UI fixtures obey the actual persisted order contract', () => {
  assert.equal(isOrder(uiOrder), true)
})

test('catalog metadata preserves exact warehouse keys and separate product groups', async (t) => {
  const enriched = {
    ...metadata,
    warehouses: [' Алматы ', '001'],
    product_categories: ['Трубы'],
    data_quality: { catalog_enrichment: {
      status: 'partial', matched: 2, total_catalog: 10, conflicts: 1,
      fetched_at: '2026-09-23T10:00:00Z', crawl_coverage: 'partial',
    } },
  }
  t.mock.method(globalThis, 'fetch', async () => json(enriched))
  assert.deepEqual(await fetchMeta(), enriched)
})

test('catalog display metadata rejects malformed lists, counters and nested objects', async (t) => {
  const invalid = [
    { ...metadata, product_categories: null },
    { ...metadata, product_categories: [{}] },
    ...[
      { status: {} }, { fetched_at: {} }, { matched: -1 }, { conflicts: '1' },
      { matched: 11, total_catalog: 10 }, { crawl_coverage: {} },
      { conflict_reasons: { brand: {} } },
    ].map((report) => ({ ...metadata, data_quality: { catalog_enrichment: report } })),
  ]
...
test('old and enriched snapshots are supported without accepting unsafe field shapes', () => {
  const value = structuredClone(order)
  const product = value.calculation.groups[0].lines[0]
  Object.assign(product, {
    product_category: 'Трубы', product_subcategory: 'Гофрированные', product_brand: 'IEK',
    product_url: 'https://ekt.kz/catalog/truby/item/', catalog_fetched_at: '2026-09-23T10:00:00Z',
    product_attributes: { Цвет: 'Красный', Материал: '<b>ПНД</b>' },
  })
  value.calculation.product_category = 'Трубы'
  value.calculation.data_quality = { catalog_enrichment: { status: 'loaded', matched: 1, total_catalog: 1 } }
  assert.equal(isOrder(order), true)
  assert.equal(isOrder(value), true)
  for (const key of ['product_category', 'product_subcategory', 'product_brand', 'product_url', 'catalog_fetched_at']) {
    const invalid = structuredClone(value)
    invalid.calculation.groups[0].lines[0][key] = { text: 'Товар' }
    assert.equal(isOrder(invalid), false, key)
  }
  for (const attributes of [null, [], { Цвет: {} }, { Количество: 2 }]) {
    const invalid = structuredClone(value)
    invalid.calculation.groups[0].lines[0].product_attributes = attributes
    assert.equal(isOrder(invalid), false)
  }
  value.calculation.product_category = {}
  assert.equal(isOrder(value), false)
})

test('queued calculation sends both source and product category without conflating them', async (t) => {
  t.mock.method(globalThis, 'fetch', async (_path, options) => {
    assert.deepEqual(JSON.parse(options.body), {
      warehouse: ' Алматы ', category: 'Категория 1', product_category: 'Трубы', explain: false,
    })
    assert.equal(options.headers.Prefer, 'respond-async')
    return json({ job_id: 'job-ekt', status: 'queued' }, 202)
  })
  assert.equal((await recommend({ warehouse: ' Алматы ', category: 'Категория 1', product_category: 'Трубы', explain: false })).job_id, 'job-ekt')
})
