...
const isStringRecord = (value) => isObject(value) && Object.values(value).every(isText)
const optional = (value, check) => value === undefined || value === null || check(value)
const catalogCounts = ['total_catalog', 'web_products', 'valid_products', 'matched', 'unmatched',
  'conflicts', 'invalid_products', 'duplicate_skus', 'duplicate_products', 'unknown_brand_matches',
  'crawl_quarantined', 'crawl_failures']
const isCatalogSummary = (value) => isObject(value)
  && optional(value.status, (status) => ['disabled', 'missing', 'invalid', 'empty_catalog', 'partial', 'loaded'].includes(status))
  && optional(value.source, isText) && optional(value.fetched_at, isText)
  && catalogCounts.every((key) => value[key] === undefined || isCount(value[key]))
  && optional(value.crawl_coverage, (coverage) => ['partial', 'complete'].includes(coverage))
  && optional(value.conflict_reasons, (reasons) => isObject(reasons) && Object.values(reasons).every(isCount))
  && !(isCount(value.matched) && isCount(value.total_catalog) && value.matched > value.total_catalog)
const isDataQuality = (value) => isObject(value)
  && optional(value.catalog_enrichment, isCatalogSummary)
...
const isLine = (line) => isObject(line) && typeof line.line_id === 'string' && line.line_id.length > 0
  ...
  && ['product_category', 'product_subcategory', 'product_brand', 'product_url', 'catalog_fetched_at']
    .every((key) => optional(line[key], isText))
  && (line.product_attributes === undefined || isStringRecord(line.product_attributes))
...
export function isRecommendation(value) {
  if (!isObject(value) || typeof value.calculation_id !== 'string' || !value.calculation_id
    ...
    || !optional(value.as_of, isText) || !optional(value.warehouse, isText) || !optional(value.category, isText)
    || !optional(value.product_category, isText) || !optional(value.data_quality, isDataQuality)
...
const isMeta = (value) => isObject(value) && isStrings(value.warehouses) && isStrings(value.categories)
  && (value.product_categories === undefined || isStrings(value.product_categories))
  ...
  && optional(value.data_quality, isDataQuality)
