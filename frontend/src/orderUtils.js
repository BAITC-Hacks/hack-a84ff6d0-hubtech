...
export function quantityError(line, rawQuantity) {
  if (rawQuantity === '' || String(rawQuantity).trim() === '')
    return 'Введите количество или 0, чтобы исключить позицию.'
  const quantity = Number(rawQuantity)
  if (!Number.isFinite(quantity) || quantity < 0)
    return 'Количество должно быть числом не меньше 0.'
  if (quantity === 0) return ''
  const minimum = Number(line.min_order_qty) || 0
  const pack = Number(line.pack_size) || 1
  if (quantity + 1e-8 < minimum)
    return `Минимальный заказ: ${minimum}. Для исключения укажите 0.`
  if (Math.abs(quantity / pack - Math.round(quantity / pack)) > 1e-8)
    return `Количество должно быть кратно ${pack}.`
  return ''
}

export function canApproveLine(line) {
  return typeof line.unit === 'string' && Boolean(line.unit.trim()) && line.unit !== 'ед. (не указана)'
}
    ...
    (totals, line) => {
      ...
      if (quantity > 0) {
        ...
        if (approvals[line.line_id]) {
          ...
          totals.approvedByUnit[unit] =
            (totals.approvedByUnit[unit] || 0) + quantity
        }
      } else {
        totals.omitted += 1
      }
...
export function buildExportPayload(result, edits, approvals, approvedOnly) {
  ...
  if (totals.invalid)
    throw new Error('Исправьте отмеченные количества перед экспортом.')
  if (approvedOnly && !totals.approved)
    throw new Error('Утвердите хотя бы одну позицию с количеством больше 0.')
  if (!approvedOnly && !totals.positions)
    throw new Error('Добавьте хотя бы одну позицию с количеством больше 0.')
  return {
    ...
    lines: lines.map((line) => {
      ...
      return {
        ...
        quantity,
        approved: quantity > 0 && Boolean(approvals[line.line_id]),
      }
    }),
  }
}
