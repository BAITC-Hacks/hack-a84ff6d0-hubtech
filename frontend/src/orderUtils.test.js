...
test('catalog links accept only HTTPS product paths on supported EKT hosts', () => {
  for (const value of [
    'https://ekt.kz/catalog/item-001/',
    'https://www.ekt.kz/catalog/item-001/',
    'https://ekt.kz:443/catalog/item-001/',
  ])
    assert.equal(safeEktUrl(value), new URL(value).href)
  for (const value of [
    undefined, null, {}, '', '/catalog/item/', '//ekt.kz/catalog/item/',
    'http://ekt.kz/catalog/item/', 'javascript:alert(1)',
    'https://ekt.kz.example.com/catalog/item/', 'https://example.com/catalog/item/',
    'https://ekt.kz@evil.example/catalog/item/',
    'https://user:password@ekt.kz/catalog/item/', 'https://ekt.kz:8443/catalog/item/',
    'https://ekt.kz/', 'https://ekt.kz/catalog/', 'https://ekt.kz/about/contacts/',
    'https://ekt.kz/catalog/../about/', 'https://ekt.kz/catalog/a/../item/',
    'https://ekt.kz/catalog/%2e%2e/item/', 'https://ekt.kz/catalog/%ZZ/',
    'https://ekt.kz/catalog/%5citem/', 'https://ekt.kz/catalog/\nitem/',
    'https://ekt.kz/catalog/%00item/', ' https://ekt.kz/catalog/item/',
  ])
    assert.equal(safeEktUrl(value), null, String(value))
})

test('unknown units stay explicit and are never added to a known unit total', () => {
  assert.equal(unitLabel(' м '), 'м')
  for (const value of [undefined, null, '', '   '])
    assert.equal(unitLabel(value), 'ед. (не указана)')
  const totals = summarizeLines([line, { ...otherLine, unit: 'шт' }], {}, {})
  assert.deepEqual(totals.unitsByUnit, { 'ед. (не указана)': 12, шт: 18 })
})
...
test('zero edit remains zero, is allowed under MOQ, and is excluded from counts', () => {
  ...
  assert.deepEqual(
    ...
    {
      units: 18,
      unitsByUnit: { 'ед. (не указана)': 18 },
      positions: 1,
      high: 1,
      approved: 1,
      approvedUnits: 18,
      approvedByUnit: { 'ед. (не указана)': 18 },
      omitted: 1,
      invalid: 0,
    },
  )
