...
test('unknown units cannot be approved, while explicit units remain usable', () => {
  for (const unit of ['', ' ', 'ед. (не указана)', undefined]) assert.equal(canApproveLine({ unit }), false)
  for (const unit of ['ед.', 'шт', 'м', 'упак']) assert.equal(canApproveLine({ unit }), true)
})

test('catalog links accept only the exact HTTPS EKT origin without credentials or custom ports', () => {
  assert.equal(safeEktUrl('https://ekt.kz/catalog/item/'), 'https://ekt.kz/catalog/item/')
  for (const value of [null, '', 'javascript:alert(1)', 'http://ekt.kz/item', 'https://ekt.kz.evil.test/', 'https://evil.test/ekt.kz', 'https://user:secret@ekt.kz/', 'https://ekt.kz:8443/']) {
    assert.equal(safeEktUrl(value), null)
  }
})
