      ...
      .map((group) => ({
        ...
        filteredLines: group.lines.filter(
          (line) =>
            ...
            (!query ||
              [line.sku, line.supplier_sku, line.name, line.warehouse, line.product_category, line.product_subcategory, line.product_brand]
                .filter(Boolean)
                .join(' ')
  ...
  return (
    <>
      ...
      <p className="result-context">
        ...
        {order.calculation.category || 'Все категории'} · сервис{' '}
        {fmt(order.calculation.service_level * 100)} % · проверка каждые{' '}
        {order.calculation.review_period_days} дн
        {order.calculation.product_category && <> · EKT: {order.calculation.product_category}</>}
      </p>
