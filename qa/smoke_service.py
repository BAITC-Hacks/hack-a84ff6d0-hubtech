...
def verify_excel(client, order, approved_only):
    ...
    assert len(rows[0]) == 37, 'Expected main catalog fields in the saved-order Excel format'
    ...
    for actual, (line, decision) in zip(exported, selected):
        ...
        for column, key in (
            ("Товарная группа ekt.kz", "product_category"), ("Подкатегория ekt.kz", "product_subcategory"),
            ("Бренд ekt.kz", "product_brand"), ("Карточка ekt.kz", "product_url"),
            ("Дата справочника ekt.kz", "catalog_fetched_at"),
        ):
            assert (actual[column] or "") == (line.get(key) or ""), (column, line["sku"])
    ...
    assert parameters["Товарная группа ekt.kz"] == (order["calculation"].get("product_category") or "Все")
    workbook.close()
    return len(selected)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="umytpa-http-qa-") as temporary:
        ...
        os.environ.update({
            ...
            "EKT_CATALOG_ENABLED": "true", "EKT_CATALOG_PATH": str(ROOT / "data" / "ekt" / "catalog.json"),
        })
        ...
        try:
            ...
            assert isinstance(metadata["product_categories"], list)
            if args.real_data:
                coverage = metadata["data_quality"]["catalog_enrichment"]
                assert 0 < coverage["matched"] <= coverage["total_catalog"]
                assert metadata["product_categories"]
            ...
            assert len(lines) > 0
            positive = [row for row in lines if row["recommended_qty"] > 0]
            line = next((row for row in positive if row.get("product_category")), positive[0])
            ...
            if args.real_data:
                filtered_params = {"warehouse": line["warehouse"], "category": line["category"],
                                   "product_category": line["product_category"], "explain": False}
                filtered_job = manager.request("/recommend", "POST", filtered_params, expected=202,
                                               headers={"Prefer": "respond-async", "Idempotency-Key": secrets.token_urlsafe(24)})
                filtered_order = await_job(manager, filtered_job["job_id"])
                filtered_lines = [row for group in filtered_order["calculation"]["groups"] for row in group["lines"]]
                assert filtered_lines and all(row["product_category"] == line["product_category"]
                                              and row["category"] == line["category"]
                                              and row["warehouse"] == line["warehouse"] for row in filtered_lines)
                assert filtered_order["calculation"]["product_category"] == line["product_category"]
                verify_excel(manager, filtered_order, False)
                print("PASS: separate category, EKT product-group and warehouse filters persist through queue, order and Excel", flush=True)
