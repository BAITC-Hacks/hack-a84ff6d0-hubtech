...
В проект выборочно добавлены скрипты очистки и подготовки `scripts/`, их тесты и инструкции. Результаты создаются локально в `data/clean/iek/` и `data/ml/iek/`; большие производные CSV исключены из Git. Основной расчёт продолжает читать Excel через `ExcelDataSource`. Файлы ML имеют другой контракт и не подключаются через `DATA_SOURCE=csv`.
...
При интеграции `alish` 23.09.2026 прошли 61 backend-тест (включая реальные Excel) и 14 тестов отдельных скриптов. Фронтенд в этой итерации не менялся. В предыдущей проверке обогащения EKT прошли 8 frontend-тестов, сборка и lint. Через интерфейс скачан и прочитан XLSX с одной утверждённой позицией: исходная рекомендация 206, ручное количество 210, товарная группа и ссылка EKT сохранены. Изменение фильтра без нового расчёта не изменило параметры экспортируемого снимка.
